    ...
    return (
      <EventEditionShell edition={edition}>
        <main className="flex min-h-[70svh] items-center justify-center px-4 text-center">
          <div className="glass-panel rounded-[2rem] p-8 font-bold text-[#1c05b3] dark:bg-slate-900/90 dark:border-slate-800 dark:text-sky-400">Loading event...</div>
        </main>
      </EventEditionShell>
    );
  }
  ...
  return (
    <EventEditionShell edition={edition}>
      <main className="mx-auto max-w-[1320px] px-3 py-5 sm:px-6 lg:px-8">
        <section className="mb-5 grid gap-3 rounded-[1.8rem] border border-white/80 dark:border-slate-800 bg-white/70 dark:bg-slate-900/90 p-4 shadow-[0_16px_36px_rgba(28,5,179,0.08)] md:grid-cols-[1fr_auto] md:items-center">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.24em] text-[#1c05b3] dark:text-sky-400">Round {round.number} of {edition.config.rounds.length}</p>
            ...
            <button
              ...
              className={`min-h-[6.5rem] rounded-[1.2rem] border p-3 text-center text-sm font-bold leading-5 transition sm:min-h-[8rem] sm:p-4 ${
                card.revealed || card.matched
                  ? card.kind === "prompt"
                    ? "border-[#bfdbfe] bg-[#e0e4ff] text-[#0f172a] dark:border-blue-500/30 dark:bg-blue-900/40 dark:text-white"
                    : "border-[#bae6fd] bg-[#f0f9ff] text-[#2406e2] dark:border-sky-500/30 dark:bg-sky-900/40 dark:text-sky-200"
        ...
        {status === "complete" && lastResult ? (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0f172a]/35 dark:bg-slate-950/80 px-4 backdrop-blur-sm">
            <div className="glass-panel max-w-lg rounded-[2rem] p-6 text-center shadow-[0_24px_60px_rgba(15,23,42,0.18)] dark:bg-slate-900/90 dark:border-slate-800">
              <TrophyIcon className="mx-auto h-10 w-10 text-[#1c05b3] dark:text-sky-400" />
              <p className="mt-4 text-xs font-bold uppercase tracking-[0.24em] text-[#1c05b3] dark:text-sky-400">Round Complete</p>
              ...
              <p className="mt-2 text-5xl font-black text-[#1c05b3] dark:text-sky-400">{formatNumber(lastResult.score)}</p>
              <p className="mx-auto mt-4 max-w-md text-sm leading-7 text-[#586074] dark:text-slate-400">{round.summary}</p>
              <button
                type="button"
                onClick={() => void continueFlow()}
                className="mt-6 inline-flex w-full items-center justify-center gap-3 rounded-full bg-gradient-to-b from-[#2406e2] to-[#1c05b3] px-6 py-4 font-bold text-white shadow-md"
              >
...
const Hud = ({ label, value }: { label: string; value: string }) => (
  <div className="rounded-[1rem] bg-white/85 dark:bg-slate-900/90 dark:border dark:border-slate-800 px-4 py-3 shadow-sm">
    ...
    <p className="mt-1 text-xl font-black text-[#1c05b3] dark:text-sky-400">{value}</p>
  </div>
);
