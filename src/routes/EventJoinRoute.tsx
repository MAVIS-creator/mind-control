      ...
      <EventEditionShell edition={edition}>
        <main className="flex min-h-[70svh] items-center justify-center px-4 text-center">
          <div className="glass-panel rounded-[2rem] p-8 dark:bg-slate-900/90 dark:border-slate-800">
            ...
            <p className="mt-4 font-bold text-[#3525cd] dark:text-indigo-400">Loading event...</p>
          </div>
        </main>
      </EventEditionShell>
    );
      ...
      <main className="mx-auto max-w-[1440px] px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
        <section className="grid items-center gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="text-center lg:text-left">
            ...
            <p className="mt-7 text-xs font-bold uppercase tracking-[0.28em] text-[#3525cd] dark:text-indigo-400">
              {edition.config.heroLabel}
            </p>
            <h1 className="mt-3 font-display text-5xl font-black tracking-[-0.07em] text-[#111c2d] dark:text-white sm:text-7xl">
              {edition.title.replace("MindGrid: Neural Clash - ", "")}
            </h1>
            <p className="mt-5 max-w-2xl text-lg leading-9 text-[#4f5568] dark:text-slate-400 lg:mx-0">
          ...
          <div className="glass-panel rounded-[2.2rem] p-5 shadow-[0_18px_42px_rgba(53,37,205,0.08)] sm:p-7 dark:bg-slate-900/90 dark:border-slate-800">
            <p className="text-xs font-bold uppercase tracking-[0.24em] text-[#3525cd] dark:text-indigo-400">{edition.config.eventLabel}</p>
            <h2 className="mt-2 text-3xl font-bold tracking-[-0.05em] text-[#111c2d] dark:text-white">Enter Event</h2>
            <div className="mt-6 space-y-4">
              <label className="block">
                ...
                <input
                  ...
                  className="mt-2 w-full rounded-[1.2rem] border border-[#dce5f6] dark:border-slate-700 bg-white/80 dark:bg-slate-900 px-4 py-4 text-lg outline-none focus:border-[#4f46e5] dark:text-white"
                  placeholder="CyberBK"
                />
              </label>
              <label className="block">
        ...
        <section className="mt-10">
          <div className="mb-5 text-center">
            <p className="text-xs font-bold uppercase tracking-[0.28em] text-[#3525cd] dark:text-indigo-400">Career paths from the seminar</p>
            <h2 className="mt-2 text-4xl font-bold tracking-[-0.05em] text-[#111c2d] dark:text-white">This event is bigger than one match.</h2>
          </div>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {edition.config.categories.map((category) => (
              <article key={category.title} className="glass-panel rounded-[1.6rem] p-5 dark:bg-slate-900/90 dark:border-slate-800">
                <h3 className="text-lg font-bold text-[#111c2d] dark:text-white">{category.title}</h3>
...
const InfoPill = ({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) => (
  <div className="rounded-[1.4rem] border border-white/80 dark:border-slate-800 bg-white/70 dark:bg-slate-900/90 p-4 shadow-[0_12px_26px_rgba(53,37,205,0.06)]">
    <div className="text-[#3525cd] dark:text-indigo-400">{icon}</div>
    <p className="mt-3 font-bold text-[#111c2d] dark:text-white">{title}</p>
