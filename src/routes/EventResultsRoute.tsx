      ...
      <main className="mx-auto flex min-h-[calc(100svh-92px)] max-w-5xl items-center justify-center px-4 py-8 sm:px-6 lg:px-8">
        <section className="w-full max-w-3xl text-center">
          <div className="glass-panel rounded-[2.4rem] p-6 shadow-[0_24px_60px_rgba(37,99,235,0.12)] sm:p-9 dark:bg-slate-900/90 dark:border-slate-800">
            <TrophyIcon className="mx-auto h-14 w-14 text-[#1c05b3] dark:text-sky-400" />
            <p className="mt-5 text-xs font-bold uppercase tracking-[0.26em] text-[#1c05b3] dark:text-sky-400">Event Complete</p>
              ...
              {run.roundResults.length ? (
                run.roundResults.map((round) => (
                  <div key={round.roundId} className="flex items-center justify-between rounded-[1.2rem] bg-white/70 dark:bg-slate-800/50 px-4 py-3 text-left">
                    ...
                    <p className="font-black text-[#1c05b3] dark:text-sky-400">{formatNumber(round.score)}</p>
                  </div>
                ))
              ) : (
          ...
          <div className="mt-5 flex flex-col justify-center gap-3 sm:flex-row">
            <Link to={`/${edition.slug}/play`} className="inline-flex items-center justify-center gap-3 rounded-full bg-gradient-to-b from-[#2406e2] to-[#1c05b3] px-8 py-4 font-bold text-white shadow-md">
              <PlayIcon className="h-5 w-5" />
              Play Again
            </Link>
            <Link to={`/${edition.slug}/live`} className="inline-flex items-center justify-center gap-3 rounded-full border border-[#cbd5e1] dark:border-slate-800 bg-white/80 dark:bg-slate-900 px-8 py-4 font-bold text-[#1c05b3] dark:text-sky-400 shadow-sm">
...
const ResultTile = ({ label, value }: { label: string; value: string }) => (
  <div className="rounded-[1.3rem] bg-white/75 dark:bg-slate-950/50 px-4 py-5">
    ...
    <p className="mt-2 text-2xl font-black text-[#1c05b3] dark:text-sky-400">{value}</p>
  </div>
);
