...
export const PublicSiteShell = ({ active, children }: PublicSiteShellProps) => (
  <div className="min-h-screen overflow-x-hidden bg-[radial-gradient(circle_at_top_left,_#e2dfff_0%,_#f9f9ff_42%,_#d4e3ff_100%)] text-[#121a2c]">
    <div className="fixed inset-0 -z-10 overflow-hidden">
      <div className="absolute -left-16 top-14 h-[24rem] w-[24rem] rounded-full bg-[#8a70ff]/18 blur-[95px]" />
      <div className="absolute right-0 top-0 h-[26rem] w-[26rem] rounded-full bg-[#64a8fe]/16 blur-[95px]" />
      <div className="absolute bottom-0 left-1/3 h-[22rem] w-[22rem] rounded-full bg-[#6b00b7]/10 blur-[110px]" />
      <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.34)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.34)_1px,transparent_1px)] bg-[size:88px_88px] opacity-40 [mask-image:linear-gradient(to_bottom,black,transparent_88%)]" />
    </div>

    <header className="sticky top-0 z-40 border-b border-[#d9def1]/70 bg-white/76 backdrop-blur-2xl">
      <div className="mx-auto flex max-w-[1500px] flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8">
        <Link to="/" className="flex min-w-0 items-center gap-3">
          ...
          <span className="font-display text-2xl font-extrabold tracking-[-0.06em] text-[#111c2d] sm:text-3xl">
            MindGrid
          </span>
        </Link>
        ...
        <nav className="order-3 flex w-full items-center justify-center gap-2 sm:order-none sm:w-auto">
          {navItems.map((item) => (
            <Link
              ...
              to={item.to}
              className={cn(
                "inline-flex items-center rounded-full px-4 py-2 text-sm font-semibold text-[#667085] transition sm:text-base",
                active === item.id && "bg-[#f0edff] text-[#3525cd] shadow-[0_10px_22px_rgba(53,37,205,0.08)]",
              )}
            >
              {item.label}
            </Link>
