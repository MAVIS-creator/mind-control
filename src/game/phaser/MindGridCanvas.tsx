...
export const MindGridCanvas = ({ state, onReveal }: MindGridCanvasProps) => {
  ...
  useEffect(() => {
    if (!hostRef.current || gameRef.current || canvasSize.width <= 0 || canvasSize.height <= 0) return undefined;
  ...
  return (
    <div
      ref={containerRef}
      className="glass-panel relative min-h-[320px] min-w-0 overflow-hidden rounded-[1.75rem] border border-white/70 shadow-[0_18px_36px_rgba(53,37,205,0.06)] sm:min-h-[420px] sm:rounded-[2.2rem] lg:h-full"
      style={{
        aspectRatio: `${Math.max(columns, 1)} / ${Math.max(rows, 1)}`,
      }}
    >
      ...
      <div
        ref={hostRef}
        className="absolute inset-2 min-w-0 overflow-hidden sm:inset-4 lg:inset-5"
      />
    </div>
  );
};
